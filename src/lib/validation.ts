...
export const animalSchema = z.object({
  ...
  estimatedAge: z.string().trim().max(100).optional(),
  colour: z.string().trim().max(120).optional(),
  identifyingFeatures: z.string().trim().max(500).optional(),
  registrationType: z.enum(["born", "purchased", "transferred-in", "existing"]).optional(),
  datePurchased: optionalDate,
  acquisitionDate: optionalDate,
  purchasePrice: optionalNumber,
  supplier: z.string().trim().max(150).optional(),
  sellerContact: z.string().trim().max(150).optional(),
  purchasedFor: z.string().trim().max(150).optional(),
  transportInformation: z.string().trim().max(1000).optional(),
...
export const healthRecordSchema = z.object({
  ...
  problem: z.string().trim().min(2, "Describe the problem, reason or observation").max(500),
  ...
  vaccineName: z.string().trim().max(300).optional(),
  dosage: z.string().trim().max(120).optional(),
  veterinaryVisit: z.boolean().optional(),
  vetName: z.string().trim().max(200).optional(),
  vetContact: z.string().trim().max(100).optional(),
  healthStatus: z.enum(["healthy", "under-observation", "sick", "injured", "recovering"]).optional(),
  followUp: z.string().trim().max(2000).optional(),
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Enter the date"),
  nextDate: optionalDate,
  notes: z.string().max(3000).optional(),
}).refine((data) => data.type !== "vaccination" || Boolean(data.vaccineName?.trim()), {
