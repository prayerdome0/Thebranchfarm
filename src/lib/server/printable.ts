...
function formatDate(value: Date | string | { seconds?: number } | null | undefined) {
  ...
  if (typeof value === "object" && typeof (value as any).seconds === "number") {
    return new Date((value as any).seconds * 1000).toLocaleDateString("en-GB", { dateStyle: "medium" });
  }
...
<body>
  <div class="toolbar"><button onclick="window.print()">Print / Save PDF</button><a href="/">Back to shop</a></div>
  <main class="sheet">
    <header>
      <div class="brand">
    ...
    <div class="totals">
      ...
      <div><span>Delivery</span><span>${input.deliveryFee ? escapeHtml(formatMoney(input.deliveryFee, currency)) : "Free"}</span></div>
