...
export function VideoCard({ video, priority = false }: { video: FarmVideo; priority?: boolean }) {
  const poster = video.posterUrl || "/media/farm-sunset.jpg";
    ...
    <article className="farm-video-card">
      <div className="farm-video-frame">
        {isRealVideo ? (
          ...
          // Animated cinematic still — shown for entries without a playable file.
      ...
      <div className="farm-video-body">
        ...
        <small>
          {formatDate(video.createdAt)} · {video.createdByName || "The Branch Farm"}
        </small>
      </div>
    </article>
  );
