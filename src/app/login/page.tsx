...
function LoginForm() {
  ...
  return <><form className="auth-form" onSubmit={submit} noValidate>{error && <div className="form-alert error"><CircleAlert size={18} /> {error}</div>}<label className="field"><span>Email</span><div className="input-with-icon"><Mail size={18} /><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" autoFocus /></div></label><label className="field"><span className="field-label-row">Password <Link href="/reset-password">Forgot password?</Link></span><div className="input-with-icon input-with-action"><LockKeyhole size={18} /><input type={show ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" placeholder="Your password" /><button type="button" onClick={() => setShow((value) => !value)} aria-label={show ? "Hide password" : "Show password"}>{show ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label><button className="button button-primary button-large button-full" disabled={loading}>{loading ? <><i className="button-spinner" /> Signing in…</> : <>Sign in <ArrowRight size={18} /></>}</button></form><p className="auth-switch">New here? <Link href="/register">Create an account</Link></p></>;
}

export default function LoginPage() {
  return <AuthShell eyebrow="Welcome to sign in." title="Sign in." description="Sign in to continue."><Suspense fallback={<div className="loading-state"><i className="loader" /> Loading…</div>}><LoginForm /></Suspense></AuthShell>;
}
