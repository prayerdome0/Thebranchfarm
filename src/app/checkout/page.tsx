  ...
  return (
    <section className="checkout-section"><div className="container checkout-container"><div className="checkout-top"><Link href="/cart"><ArrowLeft size={17} /> Back to cart</Link><div className="secure-checkout"><LockKeyhole size={16} /> Secure checkout</div></div><div className="checkout-title"><span className="eyebrow">Almost there</span><h1>Delivery &amp; agreement</h1><p>Official prices are checked again by the secure order service before your order is recorded.</p></div>
      ...
      <form onSubmit={submit} className="checkout-layout" noValidate><div className="checkout-form-column">
        ...
        <section className="checkout-card agreement-card" id="purchase-agreement"><div className="checkout-card-head"><span>3</span><div><h2>Purchase agreement &amp; signature</h2><p>Confirm the order terms before your order is recorded.</p></div></div><div className="agreement-summary" id="agreement-copy"><ShieldCheck size={21} /><p>I confirm that the information above is correct. I agree to purchase the listed products at the stated official price and confirm that I have ordered the quantities shown. This signed agreement is attached to the order record.</p></div><label className="check-field"><input id="agreementAccepted" name="agreementAccepted" type="checkbox" checked={form.agreementAccepted} onChange={(e) => update("agreementAccepted", e.target.checked)} aria-describedby="agreement-copy" aria-invalid={Boolean(errors.agreementAccepted)} /><span><i><Check size={14} /></i><span>I have read and accept the purchase agreement.</span></span></label>{errors.agreementAccepted && <small className="field-error standalone-error" role="alert">{errors.agreementAccepted}</small>}<SignaturePad value={form.signature} onChange={(value) => update("signature", value)} disabled={submitting} />{errors.signature && <small className="field-error standalone-error" role="alert">{errors.signature}</small>}</section>
      </div>
