...
export default function OrderSuccessPage() {
  ...
  const orderNumber = decodeURIComponent(params.orderNumber).toUpperCase();
  const [data, setData] = useState<SuccessData | null>(null);
  const [copied, setCopied] = useState(false);
  useEffect(() => { const raw = sessionStorage.getItem(`order-success:${orderNumber}`); if (raw) { try { setData(JSON.parse(raw)); } catch {} } }, [orderNumber]);
  ...
  const copy = async () => { await navigator.clipboard.writeText(orderNumber); setCopied(true); window.setTimeout(() => setCopied(false), 1800); };
